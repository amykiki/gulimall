...
import daily.boot.gulimall.common.httpclient.BaseHttpMessage;
...
public class AliSignUtil {
     ...
     */
    public static String sign(HttpClientReq request , AliRequestConfig config) {
        try {
            String signString = buildStringToSign(request);
            ISignerFactory signerFactory = SignerFactoryManager.findSignerFactory(config.getSignatureMethod());
            
            if (null == signerFactory) {
                throw new SdkException("unsupported signature method:" + config.getSignatureMethod());
            }
            ...
            try {
                return signer.sign(signString, config.getAppSecret());
            } catch (Exception e) {
     ...
     */
    public static String buildStringToSign(HttpClientReq apiRequest) {
        
        StringBuilder sb = new StringBuilder();
        sb.append(apiRequest.getHttpMethod().getValue()).append(SdkConstant.CLOUDAPI_LF);
        
        //如果有@"Accept"头，这个头需要参与签名
        if (apiRequest.getFirstHeaderValue(HttpConstant.HTTP_HEADER_ACCEPT) != null) {
            sb.append(apiRequest.getFirstHeaderValue(HttpConstant.HTTP_HEADER_ACCEPT));
        }
        sb.append(SdkConstant.CLOUDAPI_LF);
        
        //如果有@"Content-MD5"头，这个头需要参与签名
        if (apiRequest.getFirstHeaderValue(HttpConstant.HTTP_HEADER_CONTENT_MD5) != null) {
            sb.append(apiRequest.getFirstHeaderValue(HttpConstant.HTTP_HEADER_CONTENT_MD5));
        }
        sb.append(SdkConstant.CLOUDAPI_LF);
        
        //如果有@"Content-Type"头，这个头需要参与签名
        if (apiRequest.getFirstHeaderValue(HttpConstant.HTTP_HEADER_CONTENT_TYPE) != null) {
            sb.append(apiRequest.getFirstHeaderValue(HttpConstant.HTTP_HEADER_CONTENT_TYPE));
        }
        sb.append(SdkConstant.CLOUDAPI_LF);
        ...
        if (apiRequest.getFirstHeaderValue(HttpConstant.HTTP_HEADER_DATE) != null) {
            sb.append(apiRequest.getFirstHeaderValue(HttpConstant.HTTP_HEADER_DATE));
        }
        sb.append(SdkConstant.CLOUDAPI_LF);
     ...
     * 将path、queryParam、formParam合成一个字符串
     */
    private static String buildResource(HttpClientReq request) {
        StringBuilder result = new StringBuilder();
        result.append(request.getPath());
        
        //使用TreeMap,默认按照字母排序
        TreeMap<String , String> parameter = new TreeMap<String , String>();
        if(null!= request.getQueries() && request.getQueries().size() > 0){
            for(Map.Entry<String , List<String>> entry : request.getQueries().entrySet()){
     ...
     */
    private static String buildHeaders(BaseHttpMessage apiHttpMessage) {
        //使用TreeMap,默认按照字母排序
